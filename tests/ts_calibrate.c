...
int main()
{
    ...
    //     printf("open tsdevice=%s Ok \n",tsdevice); //forlinux add for debug

	if (ts_config(ts)) {
		perror("ts_config");
                printf("ts_config fail \n ");
		exit(1);
	}
	...
	if (open_framebuffer()) {
		close_framebuffer();
                printf("open_framebuffer fail ");
		exit(1);
	}

     //    printf("open_framebuffer success \n");
